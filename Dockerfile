...
RUN DEFAULT_PY=$(which python3) && \
    ...
    PYTHONHOME= $DEFAULT_PY -m pip install -e /vllm-workspace/vllm-ascend/ --extra-index https://download.pytorch.org/whl/cpu/ && \
    PYTHONHOME= $DEFAULT_PY -m pip uninstall -y triton triton-ascend && \
    PYTHONHOME= $DEFAULT_PY -m pip install triton-ascend==3.2.1 --extra-index-url https://mirrors.huaweicloud.com/ascend/repos/pypi && \
    PYTHONHOME= $DEFAULT_PY -m pip cache purge
